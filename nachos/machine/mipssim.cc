...
void Machine::Run() {
	...
	for (;;) {
		...
		if (singleStep && (runUntilTime <= g_stats->getTotalTicks())) Debugger();
	}
}
...
int Machine::OneInstruction(Instruction *instr) {
	...
	if (DebugIsEnabled('m')) {
		...
		printf("Thread %s At PC = 0x%x: ", g_current_thread->GetName(),
			   int_registers[PC_REG]);
		if (instr->opCode == OP_BEQ || instr->opCode == OP_BGEZAL ||
				instr->opCode == OP_BGEZ || instr->opCode == OP_BGTZ ||
				instr->opCode == OP_BLEZ || instr->opCode == OP_BLTZAL ||
				instr->opCode == OP_BLTZ || instr->opCode == OP_BNE ||
				instr->opCode == OP_JAL || instr->opCode == OP_J ||
				instr->opCode == OP_JALR || instr->opCode == OP_JR ||
				instr->opCode == OP_BC1F || instr->opCode == OP_BC1T) {
			// In case of a branch, extra is not directly the offset
			printf(stri->string, IndexToAddr(instr->extra),
				   TypeToReg(stri->args[1], instr), TypeToReg(stri->args[2], instr));
		} else {
			// Normal instruction
			printf(stri->string, TypeToReg(stri->args[0], instr),
				   TypeToReg(stri->args[1], instr), TypeToReg(stri->args[2], instr));
		}
	...
	case OP_ADDIU:
		int_registers[(int)instr->rt] =
			int_registers[(int)instr->rs] + instr->extra;
		break;

	case OP_ADDU:
		int_registers[(int)instr->rd] =
			int_registers[(int)instr->rs] + int_registers[(int)instr->rt];
		break;
	...
	case OP_ANDI:
		int_registers[(int)instr->rt] =
			int_registers[(int)instr->rs] & (instr->extra & 0xffff);
		break;
	...
	case OP_LBU:
		...
		if (!mmu->ReadMem(tmp, 1, &value, false)) return 0;
	...
	case OP_LHU:
		...
		if (!mmu->ReadMem(tmp, 2, &value, false)) return 0;
	...
	case OP_LW:
		...
		if (!mmu->ReadMem(tmp, 4, &value, false)) return 0;
		nextLoadReg = instr->rt;
		nextLoadValue = value;
		break;
	...
	case OP_OR:
		int_registers[(int)instr->rd] =
			int_registers[(int)instr->rs] | int_registers[(int)instr->rs];
		break;
	...
	case OP_SB:
		if (!mmu->WriteMem(
					(unsigned)(int_registers[(int)instr->rs] + instr->extra), 1,
					int_registers[(int)instr->rt]))
			return 0;
		break;
	...
	case OP_SH:
		if (!mmu->WriteMem(
					(unsigned)(int_registers[(int)instr->rs] + instr->extra), 2,
					int_registers[(int)instr->rt]))
			return 0;
		break;
	...
	case OP_SRA:
		int_registers[(int)instr->rd] =
			int_registers[(int)instr->rt] >> instr->extra;
		break;
	...
	case OP_SW:
		if (!mmu->WriteMem(
					(unsigned)(int_registers[(int)instr->rs] + instr->extra), 4,
					int_registers[(int)instr->rt]))
			return 0;
		break;
	...
	case OP_SWL:
		...
		if (!mmu->ReadMem(tmp & ~0x3, 4, &value, false)) return 0;
		switch (tmp & 0x3) {
		case 0:
		...
		case 1:
			value = (value & 0xff000000) |
					((int_registers[(int)instr->rt] >> 8) & 0xffffff);
			break;
		case 2:
			value = (value & 0xffff0000) |
					((int_registers[(int)instr->rt] >> 16) & 0xffff);
			break;
		case 3:
			value = (value & 0xffffff00) |
					((int_registers[(int)instr->rt] >> 24) & 0xff);
			break;
		}
		if (!mmu->WriteMem((tmp & ~0x3), 4, value)) return 0;
		break;
	...
	case OP_LDC1:
		...
		if (!mmu->ReadMem(tmp, 4, &value, false)) return 0;
		float_registers[(int)instr->ft] = value;
		if (!mmu->ReadMem(tmp + 4, 4, &value, false)) return 0;
	...
	case OP_SDC1:
		if (!mmu->WriteMem(
			...
			return 0;
		if (!mmu->WriteMem(
					(unsigned)(int_registers[(int)instr->rs] + instr->extra + 4), 4,
	...
	case OP_CVT_S_W:
		*((float *)&float_registers[(int)instr->fd]) =
		...
		break;
	case OP_CVT_W_S:
		float_registers[(int)instr->fd] =
			(int)(*((float *)&float_registers[(int)instr->fs]));
		break;
	case OP_CVT_D_W:
