...
*/
//----------------------------------------------------------------------
PendingInterrupt::PendingInterrupt(VoidFunctionPtr func, int64_t param, Time t,
                                   IntType kind) {
  handler = func;
  arg = param;
  when = t;
...
IntStatus Interrupt::SetStatus(IntStatus now) {
  ...
  ASSERT((now == INTERRUPTS_OFF) ||
         (inHandler == false));  // interrupt handlers are
...
*/
//----------------------------------------------------------------------
void Interrupt::Schedule(VoidFunctionPtr handler, int64_t arg, int fromNow,
                         IntType type) {
  Time when;
...
bool Interrupt::CheckIfDue(bool advanceClock) {
  ...
  if (g_machine != NULL) g_machine->DelayedLoad(0, 0);
