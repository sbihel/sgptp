...
AddrSpace::AddrSpace(OpenFile *exec_file, Process *p, int *err) {
  ...
  *err = 0;
  ...
  int i;
  for (i = 0; i < elfHdr.e_shnum; i++) SwapELFSectionHeader(&section_table[i]);
  ...
  for (i = 0; i < elfHdr.e_shnum; i++) {
    ...
    if (section_table[i].sh_size <= 0) continue;
  ...
  for (i = 0; i < elfHdr.e_shnum; i++) {
    ...
    if (!(section_table[i].sh_flags & SHF_ALLOC)) continue;
  ...
  if (translationTable != NULL) {
    ...
    for (i = 0; i < freePageId; i++) {
      ...
      if (translationTable->getBitValid(i))
        g_physical_mem_manager->RemovePhysicalToVirtualMapping(
...
int AddrSpace::Alloc(int numPages) {
  ...
  // to succeed
  if (freePageId + numPages >= translationTable->getMaxNumPages()) return -1;
...
OpenFile *AddrSpace::findMappedFile(int32_t addr) {
  printf(
      "**** Warning: method AddrSpace::findMappedFile is not implemented "
      "yet\n");
  exit(-1);
}
